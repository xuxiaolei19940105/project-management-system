<template>
    <div class="login-wrap">
        <div class="ms-login">
            <div class="ms-title01">
                <img src="../../assets/img/logo-bg.png" height="45" width="360" />
            </div>
                ...
                this.$api.task.login(paramdata).then(response => {
                    ...
                    if (responsevalue) {
                        if (responsevalue.data && responsevalue.data != '') {
                            ...
                            let roleArr = [];
                            console.log(returndata.roleList)
                            returndata.roleList.map(v => {
                                v.menuList.map(val => {
                                    roleArr.push(val.id);
                                });
                            });
                            roleArr = new Set(roleArr);
                            localStorage.setItem('ms_role', JSON.stringify(roleArr));
